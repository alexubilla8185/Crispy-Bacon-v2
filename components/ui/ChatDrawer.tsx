...
export function ChatDrawer({ isOpen, onClose, documentContext }: ChatDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        className={`fixed top-0 right-0 h-full w-full sm:w-96 bg-background border-l border-foreground/10 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 flex flex-col ${
        ...
        <div className="p-4 border-t border-foreground/10 bg-background">
          <form onSubmit={handleSubmit} className="flex items-center gap-2">
            <input
              ...
              disabled={isLoading}
              className="flex-1 bg-foreground/5 border border-foreground/10 rounded-full px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-50"
            />
            <button
              type="submit"
