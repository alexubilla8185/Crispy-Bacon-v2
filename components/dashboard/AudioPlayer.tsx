...
export function AudioPlayer({ audioPath }: AudioPlayerProps) {
  ...
  return (
    <div className="bg-foreground/5 border border-border p-4 rounded-[24px] flex items-center gap-4">
      {signedUrl && (
        <audio
          ref={audioRef}
          src={signedUrl}
      ...
      <button onClick={toggleSpeed} className="px-3 py-1.5 rounded-[16px] bg-background border border-border text-xs font-mono hover:bg-foreground/5 transition-colors">
        {playbackRate}x
      </button>
      ...
      <button 
        ...
        disabled={isLoading}
        className="w-10 h-10 rounded-full bg-background border border-border flex items-center justify-center hover:bg-foreground/5 transition-colors disabled:opacity-50"
      >
